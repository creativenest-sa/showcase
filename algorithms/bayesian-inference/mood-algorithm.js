...
// ── Full compute ──────────────────────────────────────────────────────────────
export function compute(answers, answerOrder, options = {}) {
  ...
  // Use learned weights if provided, fall back to designed weights
  const weights = options.learnedWeights || QUESTION_WEIGHTS;

  // Weighted 3D sum with recency
  const userCoord = weightedSum(answers, ANSWER_MAP, weights, answerOrder);

  // Threshold neighbours with signature tie-break
  let neighbors = findNearestNeighbors(userCoord, MOOD_MAP, answers);

  // Blend in Bayesian scores if provided and blend weight > 0
  if (options.bayesScores && options.bayesBlendWeight > 0) {
    const bw = options.bayesBlendWeight;
    const sw = 1 - bw;
    // Re-weight neighbours: blend spatial weight with Bayesian score
    const total = neighbors.reduce((s, n) => {
      const bayes = options.bayesScores[n.mood.name] || 0;
      return s + (n.weight * sw + bayes * bw);
    }, 0);
    neighbors = neighbors.map(n => {
      const bayes = options.bayesScores[n.mood.name] || 0;
      return { ...n, weight: (n.weight * sw + bayes * bw) / total };
    });
